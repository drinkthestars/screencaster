 ...
 */
CastPlayer.prototype.loadMedia = function(mediaURL) {
  if (!this.session) {
...
CastPlayer.prototype.onMediaDiscovered = function(how, mediaSession) {
  ...
  console.log("SUCCESS MEDIA SESSION !!!!!!!!");
  ...
  if( this.localPlayerState == PLAYER_STATE.PLAYING ) {
    ...
    var vi = document.getElementsByTagName('video_image');
...
CastPlayer.prototype.onLoadMediaError = function(e) {
  console.log("ON LOAD MEDIA ERROR!!!!!!");
  console.log(e);
  this.castPlayerState = PLAYER_STATE.IDLE;
  // update UIs
  this.updateMediaControlUI();
...
CastPlayer.prototype.playMediaLocally = function(currentTime) {
  ...
  var videoURL = vi.getAttribute('src');
  console.log("YAYYYYY LOOK AT THE VID URL !!!!!!!!!!!!!!!!!!!!!!!!!! - ", videoURL);
  ...
  if( this.localPlayerState != PLAYER_STATE.PLAYING && this.localPlayerState != PLAYER_STATE.PAUSED ) {
    this.mediaContents[this.currentMediaIndex]['sources'][0] = videoURL || "http://c-854d2bab61.a-snlprod.i-42e4a238.http.atlas.cdn.yimg.com/snlprod/lotus/aeb6cd94-bd2e-4a43-8fb0-a5622dc0c960_21zfQkpRFFbf1_8_0.mp4?b=8460&m=video%2fmp4&x=1393365769&s=8402d8ee6674c8aab4d98ae87eee366e";
    this.localPlayer.src = this.mediaContents[this.currentMediaIndex]['sources'][0];
    console.log("!!!!!!");
    console.log(this.mediaContents[this.currentMediaIndex]['sources']);
...
CastPlayer.prototype.playMedia = function() {
  console.log("WE CALLED PLAY MEDIA");
...
CastPlayer.prototype.pauseMedia = function() {
  console.log("WE CALLED PAUSE MEDIA");
  if( !this.currentMediaSession ) {
...
CastPlayer.prototype.updateDisplayMessage = function() {
  ...
  console.log("[DEBUG] " + this.castPlayerState + " on " + this.session.receiver.friendlyName);
}
...
CastPlayer.prototype.updateMediaControlUI = function() {
  if( this.deviceState == DEVICE_STATE.ACTIVE ) {
    // document.getElementById("stopcast").style.display = 'block';
    // document.getElementById("startcast").style.display = 'none';
    var playerState = this.castPlayerState;
  }
  else {
    // document.getElementById("startcast").style.display = 'block';
    // document.getElementById("stopcast").style.display = 'none';
